...
/// заявки» компилировалась. Здесь это исправлено:
///
/// ```compile_fail
...
pub trait Currency: sealed::Sealed {
    ...
    /// Сколько знаков после запятой: USD — 2, EUR — 2.
...
// копировать в нём нечего. Часть 1 так же поступает с `Id<Tag>`.
...
/// Номинал заявки. Главный пример раздела про вход и выход:
