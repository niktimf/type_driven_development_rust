...
/// их принимает детерминированный симулятор, который не может отказывать.
#[derive(Debug, Default)]
...
mod tests {
    ...
    /// Предыдущий тест проверяет только `is_ok()`. Здесь проверяется тип:
    /// у `TradingApp` и `BacktestApp` разные ассоциированные типы ошибки,
    /// потому что за ними стоят разные провайдеры.
    ///
    /// `Infallible` — тип без значений, и exhaustive `match` с пустой веткой
    ...
    /// у него есть варианты со значениями, и эта же строка перестала бы
    /// компилироваться (E0004, non-exhaustive match). Если контекст выбрал
    /// не того провайдера, тест не проходит компиляцию.
    #[test]
