//! Кульминация: одна способность — несколько реализаций, выбор за контекстом.
//!
//! Раздел про трейты поставил проблему когерентности: `impl ExchangeClient
//! for RestExchange` в программе может быть ровно один. А способов отправить
//! заявку нужно несколько — боевая REST-площадка, FIX-сессия, детерминированный
//! симулятор для бэктеста. Обычный трейт с обычным `impl` тут не работает: три
//! `impl ExchangeClient for RestExchange` в одном крейте — это ошибка
...
//!   (`SubmitViaRest`, `SubmitViaFix`, `SubmitToSimulator`). Когерентность не
//!   нарушается: `impl OrderSubmitter<Context> for SubmitViaRest` у каждого
//!   провайдера ровно один, конфликтовать нечему.
//!
//! Выбор провайдера — это `delegate_components!` на конкретном контексте
//! (`TradingApp`, `BacktestApp`). Контекст решает, кто отправляет заявку,
//! ровно один раз, декларативно, а не через `enum` или динамический диспетчер.
...
/// Абстрактный тип идентификатора после отправки. У каждого контекста свой:
/// у REST — число, у FIX — строка, у симулятора для бэктеста тоже число, но
/// это не одно и то же число, что видно по ассоциированному типу ошибки.
#[cgp_type]
...
/// абстрактный тип ошибки из `cgp::prelude`, тот же приём, что и для `OrderId`.
    ...
    Context: HasOrderIdType<OrderId = RestOrderId> + HasErrorType<Error = HttpExchangeError>,
{
    fn submit_order(
        ...
        // В реальном коде соединение с площадкой живёт в контексте и достаётся
...
// провайдер за ним стоит. Это декларация, а не логика: перечитывается один
// раз при компиляции, никакого `match` по бирже во время исполнения.
