...
///
/// Стирание одностороннее.
/// `Display` умеет только печатать нативный id в строку (`EC::ExchangeOrderId -> ErasedOrderId`);
/// обратной операции у `Display` нет и быть не может — из `"42"` не восстановить,
/// был ли это `u64` или байт строки другого формата.
/// Поэтому разбор обратно не выводится, а требуется явным bound-ом `TryFrom<ErasedOrderId>`:
/// площадка, которая его не реализовала, не пройдёт в `Router::add`.
    ...
    fn submit_order(&self, order: &DraftOrder<Usd>) -> Result<ErasedOrderId, RouterError> {
        self.0
            ...
            .map(|id| ErasedOrderId {
                ...
                raw: id.to_string().into(),
            })
            // `format!("{e:?}")` схлопывает структуру ошибки площадки
            // (коды, тексты из `HttpExchangeError`/`FixReject`/`RevertReason`) в строку.
            // Это компромисс: `RouterError` общий для всех площадок, и поля под
            // конкретную структуру ошибки в нём нет — остаётся текст.
