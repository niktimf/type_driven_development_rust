...
/// Тип заявки. Параметр `Quote` пришёл сюда вместе с [`Price`] — тот самый
/// каскад, о котором предупреждает раздел про минимальные bound-ы: одному типу
/// добавили валюту, и параметр протёк во всё, что цену держит.
...
// Тот же приём, что часть 1 применяет к `Id<Tag>`.
...
/// Рабочая заявка: площадка приняла. Идентификатора два, и это не избыточность.
///
...
///
/// Живут в публичном API намеренно: валидный [`Quantity`] собирается только
