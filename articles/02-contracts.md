...
- `bids.len() != asks.len()` представимо, хотя для снимка это бессмыслица: длины двух сторон ничем не связаны;
...
Написать `where DEPTH >= 1` на стабильном Rust нельзя, зато можно свести условие к обычному trait bound:
...
Фичи аддитивны и объединяются по всему графу зависимостей: если два крейта воркспейса подключат общий крейт со стаканом — один с фичей `depth5`, другой с `depth10`, — cargo включит обе, и два `#[cfg]`-определения одной константы просто не соберутся.
