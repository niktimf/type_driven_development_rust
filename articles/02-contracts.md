...
Каждая новая биржа — правка всех таких `match`-ей, которые расползутся по коду ровно так, как мы видели в части 1.
Плюс список бирж жёстко зафиксирован в `enum`: пользователь вашей библиотеки не подключит свою, не залезая в исходник.

В части 1 `enum` отлично кодировал «одно из» для данных с закрытым набором форм,
...
impl<Quote: Currency> DraftOrder<Quote> {
    ...
    {
        let exchange_id = exchange.submit_order(&self)?;
...
Здесь он окупается — сложить доллары с евро больше не выйдет, но мы получили усложнение сигнатур.
...
К `submit_batch` мы ещё вернёмся дважды: в разделе про ассоциированные типы у площадки появится лимит на размер пакета, а в разделе про const generics мы попробуем связать этот лимит с длиной массива — и упрёмся в стену.
...
Валют же в домене конечный список, и задаёт его не пользователь библиотеки.
...
Либо тип одной из площадок, и тогда остальные контракты выполнить не смогут: 32 байта в `u64` не помещаются.
...
Выход — не размазывать унификацию по всему коду, а сделать её одним явным слоем на границе роутера:
...
- `sqlx::Database` описывает СУБД целиком: `type Connection`, `type Row`, `type Arguments<'q>` — последний уже GAT, в продакшене и на стабильном Rust.
...
### Проблема: глубина стакана известна заранее, но живёт в рантайме
...
Тому, кто ловит дисбаланс объёмов или прячущиеся айсберги, нужно десять или двадцать — на одном уровне дисбаланса не видно.
Фид под каждую подписку свой: top-of-book отдаёт один уровень, L2 — пять, десять или двадцать.

Обе стратегии живут в одном процессе, и снимок обеим приходит одного и того же типа.
При этом глубина известна ещё до первого байта данных: её задаёт конфиг подключения, и в рантайме она не меняется.
...
- `bids.len() != asks.len()` представимо, хотя для снимка это бессмыслица — та же болезнь независимых полей, что и у `Option`-ов в части 1;
- две аллокации на снимок, а снимки идут тысячами в секунду;
- компилятор не знает длину: каждый доступ с проверкой границ, цикл не развернуть.
...
- `OrderBook<1>` и `OrderBook<10>` — разные типы. Снимок на пять уровней в функцию, которой нужны десять, не передать: `error[E0308]: expected OrderBook<10>, found OrderBook<5>`.
...
`spread` выше молча предполагает, что уровень хотя бы один. Но `OrderBook<0>` — валидный тип, и на нём тот же код упадёт. Написать `where DEPTH >= 1` на стабильном Rust нельзя, зато можно свести условие к обычному trait bound:
...
Поля приватные, других конструкторов нет — значит, `OrderBook<0>` не собрать, и `spread` безопасен по построению. Это тот же smart constructor из части 1, только проверяет он не значение, а параметр типа. Сам `SupportedDepth` уместно запечатать: список поддерживаемых глубин задаёт фид, а не пользователь.

### Стена: арифметика в параметрах

Вот и обещанное третье появление `submit_batch`. Дефолтный метод из раздела про трейты брал срез и ничего не знал про лимиты. Лимит появился в предыдущем разделе — `Exchange::MAX_BATCH`, ассоциированная константа. Логично свести их вместе: выразить пакет массивом и проверить размер типом.
...
Сравнения и арифметика над const-параметрами (`N <= MAX`, `[T; N + 1]`, `N * 2`) в `where`-клаузе на стабильном Rust недоступны — это `generic_const_exprs`, он до сих пор в nightly. Про то, как это выглядит на nightly и что ещё там лежит, — в части 4.
...
Обратиться к ассоциированной константе типа-параметра внутри `const`-блока стабильный Rust позволяет — nightly для этого не нужен. Но проверка получается не той, на которую хочется рассчитывать. Вот что видно на компиляторе, если инстанцировать функцию с `N` больше лимита:
...
Это post-monomorphization error: она возникает не на сигнатуре, а при инстанцировании функции, и `cargo check` её действительно не видит — нужна полная сборка, и только если этот код вообще дошёл до кодогенерации. Для приложения приемлемо, для публичной библиотеки — плохая идея: ошибку найдёт не автор, а тот, кто библиотеку подключил.
...
`&'static str`, `f64`, свои структуры в const-параметры не годятся (`adt_const_params` — nightly). Поэтому `NAME: &'static str` у нас ассоциированная константа, а не параметр типа: это ровно та граница, где заканчиваются const generics и начинаются ассоциированные константы.
...
Каждое значение `DEPTH` — отдельная копия всего кода, который по нему обобщён. Параметризовать по глубине стакан — нормально; параметризовать по ней весь пайплайн стратегии — значит размножить пайплайн.

**Не путайте с cargo-фичами.**
Мысль «сделаю фичу `depth10`, а в другой сборке `depth5`» приходит быстро и не работает.
Фичи аддитивны и объединяются по всему графу зависимостей: если один крейт подключит вашу библиотеку с `depth5`, а другой с `depth10`, cargo включит обе, и два `#[cfg]`-определения одной константы просто не соберутся.
Отключить чужую фичу нельзя — включение необратимо.
Но дело даже не в этом: фича даёт одно значение на всю сборку, а const-параметр — сколько угодно значений в одном бинаре.
Маркет-мейкер с `OrderBook<Usd, 1>` и детектор дисбаланса с `OrderBook<Usd, 10>` работают в одном процессе; с фичей пришлось бы собирать два бинаря, и глубина всё равно осталась бы глобальной.
Фичи — чтобы включить или выключить целую необязательную часть: поддержку `serde`, ещё один протокол. Аддитивно, а не «или-или».

**Не тащите в тип то, что действительно динамическое.**
Глубина фида фиксирована конфигом — ей место в типе. Число заявок, вычитанных из очереди за итерацию, — нет: это рантайм-величина, и `Vec` для неё честнее.
...
- В `std` const generics видны в массивах: до Rust 1.51 трейты для `[T; N]` генерировались макросом до 32 элементов, теперь есть общий `impl<T, const N: usize>` — отсюда `IntoIterator` для массивов, `TryFrom<Vec<T>> for [T; N]`, `array::from_fn`.
...
Про правило когерентности мы уже говорили: `impl ExchangeClient for RestExchange` в программе ровно один. Пока реализация одна, это благо. Но чем крупнее система, тем чаще одну и ту же способность хочется иметь в нескольких вариантах:
...
Обычный ответ — newtype-обёртки (`RestExchange`, `RestSandbox`, `WsExchange`) плюс generic-параметры, которые расползаются по сигнатурам. Плюс ассоциированные типы фиксируются вместе с `impl`-ом: `type Error` у площадки один и тот же и в проде, и в тесте.

### Решение: контракт отдельно, реализация отдельно, сборка отдельно

CGP (Context-Generic Programming) расщепляет трейт на две части: consumer-трейт, который зовёт прикладной код, и provider-трейт, который реализуют. Реализации становятся самостоятельными типами, а не «привязками к контексту», и их может быть сколько угодно. Выбор конкретной реализации выносится в отдельную декларацию — по одной на контекст (приложение, тест, деплой):
...
Реализации — отдельные типы, а не `impl` «для контекста». Каждый провайдер реализует сгенерированный provider-трейт ровно один раз, поэтому когерентности нечего нарушать:
...
Две вещи в этих сигнатурах стоит заметить. Метод провайдера принимает `context: &Context`, а не `&self`: сам провайдер — тип нулевого размера, состояния он не держит, всё нужное лежит в контексте. И `type Error` у симулятора — `Infallible`, тот самый пустой тип из части 1: отказать он не может в принципе, и это записано в типе, а не в комментарии.
...
Это и есть та самая диагностика, о которой речь ниже: ошибка приезжает про типы, которых вы не писали.
...
Код, который зовёт `self.submit_order(order)`, при этом не меняется. Никакого `dyn`, никакой рефлексии: всё разрешается на этапе компиляции и сворачивается в прямые вызовы, а неиспользованная реализация в бинарь не попадает. Ассоциированные типы тоже становятся частью сборки: `HasErrorType` позволяет контексту выбрать свою ошибку, не трогая логику.

Стоит держать в голове три вещи. CGP работает на стабильном Rust, но требует свежий компилятор. Синтаксис макросов заметно менялся от версии к версии — сниппеты выше писались под 0.7, и сверяться стоит с актуальной книгой, а не с постами двухлетней давности; заодно у макросов есть более «сахарная» форма записи (`#[cgp_impl]`, `#[uses]`), которую я здесь не показываю — она короче, но неявно переписывает `Self` и позиции генериков, а для первого знакомства это лишнее. И главное — диагностика: ошибка в проводке компонентов приезжает как стена сообщений о сгенерированных типах, поэтому у проекта есть отдельный `cargo-cgp`, который переписывает их в человеческий вид.

Авторы сами очерчивают границу применимости, и она разумная: одна реализация — обычный трейт или функция; маленький закрытый набор вариантов — `enum` и `match`; выбор в рантайме (плагины, конфиг, гетерогенные коллекции) — `dyn Trait`. CGP окупается там, где реализаций реально несколько и выбор принадлежит контексту. Из заметного продакшена — hermes-sdk, новая версия IBC-релеера Informal Systems, откуда парадигма и выросла.

## Итог части 2 и что дальше

Часть 1 отвечала на вопрос «какие бывают значения». Часть 2 — на вопрос «кто с кем и на каком языке разговаривает».

Трейт задаёт контракт: что требуется от стороны и что она обещает взамен. Ассоциированные типы уточняют, чьи типы участвуют в разговоре: вход выбирает вызывающий, выход — реализация. Const generics поднимают в тип последнее, что обычно остаётся в рантайме, — размеры. А sealed-трейты и bound-ы на маркерах закрывают дыры, которые часть 1 оставила открытыми: фантомный параметр теперь не принимает что попало и умеет отдавать данные.

Цена та же, что и раньше, и платить её надо осознанно. Bound-ы расползаются по сигнатурам, мономорфизация растит бинарь и время сборки, а ошибки в generic-коде читаются хуже, чем в конкретном. Абстракция окупается там, где реализаций действительно больше одной, — и мешает там, где она одна и другой не предвидится.

До сих пор все проверки жили в пределах одного типа: этот `Price` положителен, этот клиент умеет `submit_order`, у этого стакана десять уровней. В части 3 соберём из типов структуры посложнее — списки на уровне типов (HList), валидаторы, работающие до запуска программы, и event sourcing, где корректность цепочки событий проверяет компилятор.