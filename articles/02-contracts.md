...
Обязуется не на словах: компилятор не соберёт `impl`, где метод пропущен или сигнатура разошлась с объявленной.
...
Все три приёма упираются в одно правило — «один `impl` на пару трейт + тип».
Есть проект, построенный вокруг того, чтобы его обойти.

## CGP

Смотреть на него полезно, даже если внедрять не собираетесь: он показывает, где у обычных трейтов потолок.
...
И главное — диагностика: ошибка в проводке компонентов приезжает как стена сообщений о сгенерированных типах, поэтому у проекта есть отдельный `cargo-cgp`, который переписывает их в человеческий вид.
...
По отдельности трейты, ассоциированные типы и const generics — три независимых механизма.
Но в статье они почти не ходили поодиночке: `submit_batch` прошёл через все три раздела, стакан держится на const-параметре с trait bound-ом и уровнях из GAT-фида, а CGP целиком построен на трейтах с ассоциированными типами.

Трейт задаёт контракт: что требуется от стороны и что она обещает взамен.
Ассоциированные типы уточняют контракт до конкретных типов: вход выбирает вызывающий, выход — реализация.
Const generics поднимают в тип последнее, что обычно остаётся в рантайме, — размеры.
Sealed-трейты и bound-ы на маркерах доделывают то, что часть 1 оставила в долг: фантомный параметр больше не принимает что попало и умеет отдавать данные.

Стоит всё это столько же, сколько приёмы части 1: bound-ы расползаются по сигнатурам, мономорфизация растит бинарь и время сборки, ошибки в generic-коде читаются хуже, чем в конкретном.
Абстракция окупается, когда реализаций больше одной, и мешает, когда она одна и другой не предвидится.

До сих пор все проверки жили в пределах одного типа: этот `Price` положителен, этот клиент умеет `submit_order`, у этого стакана десять уровней.
