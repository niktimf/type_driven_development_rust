...
В Части 1 типы держали в узде состояние: какие значения возможны, какие комбинации допустимы, в каком порядке идут переходы.
Но программа — это не только данные. Это ещё и компоненты, которые общаются между собой: заявка уходит на биржу, котировки приходят из фида, риск-проверка пропускает или отклоняет.
У каждого такого стыка есть контракт — что нужно от партнёра и что он обещает взамен.

В Rust контракт задают трейтом.
Возьмём стык прямо из Части 1: `DraftOrder::submit` отправлял заявку «на биржу» — но на какую?
...
Но роль у всех одна: принять заявку и вернуть либо идентификатор, либо отказ.

## Трейты как контракты

### Проблема: захардкоженный бэкенд

Пока биржа одна, всё просто — зовём её напрямую:

```rust
fn route(order: DraftOrder, binance: &Binance) -> Result<BinanceOrderId, BinanceError> {
...
fn route(order: DraftOrder, backend: &Backend) -> Result</* ??? */, /* ??? */> {
    match backend {
        Backend::Binance(b) => b.submit(order),
        Backend::Nyse(n) => n.submit(order),
    }
}
```

Затык виден уже на типе возврата: у Binance свой `OrderId` и свой `Error`, у NYSE — свои, а `match` обязан вернуть что-то одно.
И это только начало. Каждая новая площадка — правка всех таких `match`-ей, а они расползутся по коду ровно так, как мы видели в Части 1.
Плюс список бирж зашит в `enum`: пользователь твоей библиотеки не подключит свою площадку, не залезая в исходник.

`enum` здесь не виноват — просто это не его задача.
В Части 1 он отлично кодировал «одно из» для данных с закрытым набором форм.
А тут набор открытый и растущий: бэкендов сколько угодно, и часть из них появится уже после того, как ты выложил код.
...
(`OrderId` и `ExchangeError` пока общие на всех — что с этим не так и как сделать их «своими на каждой бирже», разберём в секции про ассоциированные типы.)
...
Новая биржа — это новый `impl`, и `route` подхватит её без единой правки; свою площадку может добавить даже пользователь библиотеки, не трогая твой код.
