...
Первое, что может прийти в голову, — `enum` по биржам и `match` на каждом вызове:
...
Три записи эквивалентны, но критерии выбора всё же есть:
...
Одна правка в одном типе. Но всё, что содержит `Price`, тоже получает параметр `Quote`, а именно: `OrderType<Quote>`, `DraftOrder<Quote>`, `InstrumentSpec<Quote>`, `Level<Quote>`.
...
К `submit_batch` мы ещё вернёмся дважды: в разделе про ассоциированные типы у площадки появится лимит на размер пакета, а в разделе про const generics мы попробуем связать этот лимит с длиной массива — и столкнёмся с проблемой.
...
Все трое — «та же площадка», но `impl` на них один. Обходят это отдельными типами (`RestExchange`, `RestSandbox`) или обёрткой-newtype, как в части 1, только теперь newtype нужен, чтобы реализовать второй `impl`.
Во что это выливается, когда вариантов становится много, — в разделе про CGP в конце статьи.

**Async в контракте.**
Настоящий `submit_order` ходит по сети, то есть асинхронен (в большинстве случаев).
...
Наш `ExchangeClient` пока задаёт всем площадкам один `OrderId` и один `ExchangeError`, но на практике у каждой площадки свои типы.

## Ассоциированные типы

Ассоциированный тип объявлен в трейте, но выбирает его реализация.
Самый распространённый пример — в `std`:
...
```
Для идентификаторов используются newtype над внутренним представлением, чтобы не путать площадки между собой.
`Error` — свой тип с данными отказа: коды и тексты от площадки.
...
У контракта появляется и третий ассоциированный тип — `Quote`, валюта котировки: площадка принимает заявки в своей валюте, и валюта черновика должна с ней совпасть.
...
В коде примеров `Level` обобщён по валюте (`Level<Quote>`); в сниппетах она зашита, чтобы параметр не шумел в каждом типе раздела.
...
Так не выйдет: `Self::Levels` — один фиксированный тип, и привязать его ко времени жизни `&self` нельзя: параметра под лайфтайм у ассоциированного типа нет.
...
```rust
trait ExchangeClient {
    type ExchangeOrderId: Debug + Clone + Send + Sync + 'static;
...
pub enum RouterError {
    ...
    UnparseableOrderId,   // понадобится ниже, на отмене
}
...
На nightly это выражают через `generic_const_exprs`: он разрешает арифметику над const-параметрами (`[T; N + 1]`, `N * 2`) и позволяет спрятать сравнение в баунд-тип; как это выглядит — в части 4.
...
Маркет-мейкер с `OrderBook<1>` и детектор дисбаланса с `OrderBook<10>` в одном процессе — работа для const-параметра, фичами такое не собрать.
...
Конфигурация здесь не динамическая, а флагами компиляции: внутри сборки она одна, компилятор инлайнит её насквозь, и аддитивность не срабатывает: наборы фич не встречаются в одном графе сборки. В HFT так и делают.
...
Число заявок, вычитанных из очереди за итерацию, — нет: это рантайм-величина, и использовать `Vec` здесь нормально.
...
У всех трёх приёмов есть ограничение — «один `impl` на пару трейт + тип».
...
Пока реализация одна, всё в порядке. Но чем крупнее система, тем чаще одно и то же поведение хочется иметь в нескольких вариантах:
...
Каждый провайдер реализует сгенерированный provider-трейт ровно один раз, поэтому конфликта реализаций не возникает:
...
Из-за этого ошибки компилятора приходят про типы, которых вы не писали; что с этим делать — ниже.
...
Абстракции увеличивают сложность кода:
