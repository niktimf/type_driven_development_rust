...
и свести их в одно место без макросов не получится (крейты, которые это делают, — во врезке ниже).
...
Состояние при этом — рантайм-значение, и недопустимый переход — не ошибка компиляции,
а `Err` из `consume`.
...
Последний релиз — 2021 год, для продакшена это довод против.
...
Проверки этой части выполняются до запуска: состав цепочки, полнота набора,
допустимость перехода — всё это сверяет компилятор при сборке, а не программа на данных.

HList записывает список проверок в тип: состав и порядок цепочки — часть сигнатуры.
`Contains` делает утверждение о списке доказуемым: шлюз без обязательной проверки не собирается.
`Valid<Checks>` превращает факт прогона в тип: непроверенная заявка не доходит до стакана.
`Apply` перечисляет допустимые переходы `impl`-ами: недопустимый не попадает в журнал.

Недостатки те же, что у generic-кода части 2, и заметнее: тип списка растёт с каждым элементом
и приходит в сообщения об ошибках целиком, индексы-свидетели добавляют параметр
