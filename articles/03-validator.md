...
Проверки этой части компилятор выполняет при сборке.
Начали со списка проверок, записанного типом: после этого состав и порядок цепочки
читаются из сигнатуры, а прогон общий на любой список.
Поверх списка появилось утверждение `Contains` с индексом,
и `Gate::new` перестал собираться без обязательных проверок.
Тот же прогон стал возвращать `Valid<Checks>`, так что в стакан попадает только заявка,
прошедшая цепочку.
В журнале событий переходы записаны `impl`-ами `Apply`, и `Filled` после `Cancelled`
наш код записать не может.

Недостатки те же, что у generic-кода части 2, и заметнее: тип списка растёт с каждым элементом,
...
Все доказательства этой части — обёртки: `PhantomData`, приватные поля,
типы-носители вроде `BookDepth` из части 2.
Сами значения при этом остаются обычными `u64` и `Decimal`:
ограничение записано рядом со значением, а не в нём.
В части 4 выйдем за стабильный Rust — там pattern types записывают ограничение
прямо в тип значения: `u32 is 1..` вместо обёртки со smart constructor-ом.
