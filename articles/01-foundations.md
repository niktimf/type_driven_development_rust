...
Вспомним, с чего начинали: «рыночный ордер с лимитной ценой», «отменённый ордер, всё ещё висящий в стакане» — состояния, которых быть не должно, но в наивной модели они представимы.
Теперь есть чем сделать их невыразимыми. Цена перестала быть голым `f64` и стала `Price` с инвариантом.
«Купить или продать» — это `Side`, а не `bool`. Тип заявки — `OrderType`, где у `Market` просто нет поля цены, а `match` обязан разобрать все варианты.
Жизненный цикл — цепочка типов, где `cancel` не вызвать на черновике, а у отменённой заявки нет методов вернуться в стакан.
Проверки, что раньше лежали в рантайме россыпью `if`-ов, переехали в систему типов — и компилятор отказывается собирать программу, которая их нарушает.

Обмен честный: типов становится больше, зато ошибки переезжают со стадии «упало в проде» на «не скомпилировалось».
Там, где ошибка стоит денег или безопасности, он выгодный — собственно, ровно с этого и начинал Jane Street.
А чувство меры — где это оправдано, а где overkill — приходит с практикой.

В Части 2 («Контракты») тот же фундамент уходит в трейты, ассоциированные типы и const generics — то, чем задают _интерфейсы_ для чужого кода.
Typestate там становится частью контракта трейта: «реализация обязана провести объект через эти состояния».