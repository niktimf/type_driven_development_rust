...
Любую программу можно разложить на три вида сущностей: данные, вычисления и действия.

Данные — инертные значения, которые сами по себе ничего не делают.
Вычисления — "чистые" функции: один и тот же вход всегда даёт один и тот же выход, без побочных эффектов.
Действия — всё, что зависит от внешнего мира и момента вызова: запрос в сервис, запись в базу, ввод пользователя.
...
А данные? Данные — отражение домена, его сущности и состояния. Поведения у них нет, и тесты на них не пишут.
Их корректность держится на типах.
Тип может не дать собрать недопустимое значение вообще — ещё до того, как дойдёт до тестов.
И тогда состояния, бессмысленные в домене — «рыночная заявка с лимитной ценой», «отменённый ордер, всё ещё висящий в стакане», — нельзя выразить в коде.
