...
В 2010 году Ярон Мински (Yaron Minsky) читал лекцию для индустриальной аудитории.
...
Позже её стали называть «Effective ML». Именно там прозвучала в той формулировке, которую потом стали цитировать далеко за пределами OCaml, фраза:
...
А с Rust эта история связана напрямую. Первый компилятор Rust Грейдон Хоар (Graydon Hoare, создатель языка) написал на OCaml — от ML-семейства Rust получил алгебраические типы данных, паттерн-матчинг, type inference и общее отношение к типам как к инструменту дизайна.

Языки можно расположить на шкале от «типы — формальность, проверяется как получится» (Python, JavaScript) до «типы — основной инструмент дизайна, всё держится на них» (Idris, Agda, Lean). Где на этой шкале Rust?

Rust взял у ML-семьи несколько ключевых вещей: ADT с исчерпывающим match, паттерн-матчинг, замыкания, итераторы-комбинаторы, трейты как наследник type classes, ассоциированные типы.
Именно эти инструменты и позволяют делать недопустимые состояния невыразимыми. Но Rust — это low-level язык, который проектировался без GC, и поэтому ему пришлось пожертвовать частью выразительной мощи системы типов ради безопасности работы с памятью.
...
Фича `std` нужна, чтобы был виден `argon2::password_hash::rand_core::OsRng`: по умолчанию `rand_core` подключается без `getrandom`, и `OsRng` оказывается за выключенной фичей.
...
Параметры выше — сильный production-набор для Argon2id. OWASP Password Storage Cheat Sheet даёт несколько профилей разной интенсивности — выбирать стоит под свой бюджет CPU/RAM и под целевое время хеширования (обычно подбирают ~100–500 мс на сервере).
...
Если в БД лежит мусор (хотя так быть не должно), вызов вернёт `Err`, и мы узнаём об этом сразу, а не в момент `verify`.
...
- **HTTP-builder**: `http::Request::builder()` и `hyper::Request::builder()` собирают запрос пошагово, и без обязательных полей `.body()` не уходит в `Result<Request, _>` с пустыми руками.
