...
В реальном коде, когда отказ временный и заявку хочется пере отправить, черновик возвращают обратно вместе с ошибкой — `submit(self) -> Result<WorkingOrder, (DraftOrder, RejectReason)>`.
...
Если из состояния нет выхода (как `Filled` или `Cancelled` у заявки), маркер можно сделать пустым enum-ом — `pub enum Filled {}` (uninhabited type из раздела).
