...
На практике же они редко работают изолированно: newtype задаёт инварианты, enum ограничивает форму состояния, uninhabited types помечают невозможные исходы, phantom types добавляют compile-time контекст, а typestate фиксирует допустимый порядок переходов.
Главная идея здесь не в том, чтобы «запихнуть всё в типы», а в том, чтобы сделать ограничения модели явными и проверяемыми компилятором там, где это действительно окупается.
