...
А с Rust эта история связана напрямую. Первый компилятор Rust Грейдон Хоар (Graydon Hoare, создатель языка) написал на OCaml. От ML-семьи Rust взял главное: ADT с исчерпывающим `match`, паттерн-матчинг, type inference, замыкания, итераторы-комбинаторы, трейты как наследник type classes, ассоциированные типы. Именно эти инструменты и позволяют делать недопустимые состояния невыразимыми.
