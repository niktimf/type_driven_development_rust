...
Влияние ML-семейства в Rust видно невооружённым глазом: ADT с исчерпывающим `match`, паттерн-матчинг, type inference, замыкания, итераторы-комбинаторы, ассоциированные типы; трейты — в духе type classes из Haskell.
...
Это родная среда для type-driven разработки: она полна инвариантов, и цена ошибки — реальные деньги.
...
Под наши три типа заявки подходит лишь несколько из восьми сочетаний флагов и двух `Option`-ов — остальные бессмысленны, но тип ничем их не запрещает, и отсекать их приходится руками в каждом куске кода.
...
Соберём картину — кто всем этим оперирует? У заявки два конца. **Клиент** знает тип на этапе компиляции и зовёт удобный конструктор: `place_market_order(qty)`, `place_limit_order(price, qty)`, `place_stop_limit_order(stop, limit, qty)` — каждый берёт ровно свои поля, и недопустимые аргументы отсекаются прямо в сигнатуре (рыночной цену не передать, у лимитной не забыть). Сигнатуры даже повторяют форму вариантов: `place_limit_order(price, …)` ↔ `Limit(price)`. Тип, зашитый в имя функции, — это нормально, удобная дверь для клиента.

**Приёмная сторона** — матчинг-движок, журнал событий, парсер входящего запроса — тип заранее не знает: заявки валятся к ней вперемешку всех видов. Угадать его функцией нельзя, остаётся прочитать тип как данные и разойтись по веткам тем самым `match` выше. Вот ради кого живёт `OrderType`: заявку, пришедшую из сети, и список разнотипных заявок в стакане выразит только сумма. Конструкторы строят, `enum` представляет.
...
В прикладном коде `unwrap()` (или `expect("infallible")`) — вполне идиоматичный выбор: коротко и понятно, а LLVM всё равно выкинет недостижимую ветку. Пустой `match` сильнее всего окупается в generic-коде и публичных API, где тип ошибки может смениться: он не паникует, а перестаёт компилироваться, указывая каждое место, где исход стал возможен.
...
Для простоты здесь reject поглощает черновик: `submit(self)` забирает заявку по значению, и при `Err` она уже `moved`. В реальном коде, когда отказ временный и заявку хочется переотправить, черновик возвращают обратно вместе с ошибкой — `submit(self) -> Result<WorkingOrder, (DraftOrder, RejectReason)>`.
...
Typestate хорошо ложится на линейный или почти линейный порядок шагов. Чем сильнее ветвится граф переходов (из каждого состояния можно почти в любое другое), тем дороже поддерживать его в типах — здесь обычный ADT (`enum Status { ... }`) с `match` чаще оказывается проще. Дело не в самом числе состояний: typestate-машины живут и с десятками состояний, пока переходы остаются упорядоченными.
