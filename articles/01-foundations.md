...
`UserId` и `OrderId` спасают от перепутанных идентификаторов.
...
- `is_buy: bool` болеет тем же, что мы разберём в ADT-секции: `true` — это «купить» или «продать»?
По типу не прочитать, а по значению — легко перепутать, нужно открывать код функции и держать в голове, что `true` — это покупка, а `false` — продажа.

`f64`, `bool` ничего не знают о домене. Вынесем смысл в типы.
...
Это случай чуть хитрее классического. Обычно smart constructor живёт на самом типе — `Price::new(value) -> Result<Price, _>`, когда инвариант самодостаточен (скажем, «непустая строка» или «число в диапазоне `0..=100`): всё нужное для проверки уже в самом значении. Здесь же инвариант зависит от внешнего контекста — шага инструмента, — поэтому конструктор переезжает туда, где этот контекст есть. Приём тот же, просто «дом» у конструктора другой.
...
Почему конструктор строгий, а не округляющий. Тихо подвинуть цену к ближайшему тику — значит изменить заявку за спиной у того, кто её выставляет: на лимитке это другая цена исполнения, живые деньги. Поэтому `price` возвращает `Err` на некратной цене. Если округление нужно — это отдельный явный шаг (`round_price`), где вызывающий сам выбирает стратегию: вниз для покупки, чтобы не переплатить, вверх для продажи. Молчаливого «округлим к ближайшему» по умолчанию здесь нет — сам факт округления должен быть виден в коде.
...
И `is_buy: bool` из биржевого примера — та же болезнь, только в одном флаге: по `true` не прочитать, «купить» это или «продать». Лечится так же — `enum` вместо примитива:
...
Typestate собрал все предыдущие приёмы в один паттерн:

- **Newtype** — каждое состояние это отдельный тип с приватным полем и своим инвариантом.
- **ADT** — набор состояний и переходов, фиксированный на этапе компиляции; только варианты одного `enum`-а здесь стали отдельными типами, а переходы — методами, потребляющими `self`.
- **Uninhabited types** — пустой `enum`-маркер у терминального состояния: на уровне типа видно, что выхода нет.
- **Phantom types** — одна структура, параметризованная состоянием, без рантайм-оверхеда.

Вспомним, с чего начинали: «рыночный ордер с лимитной ценой», «отменённый ордер, всё ещё висящий в стакане» — состояния, которых быть не должно, но в наивной модели они представимы. Теперь есть чем сделать их невыразимыми. Цена перестала быть голым `f64` и стала `Price` с инвариантом. «Купить или продать» — это `Side`, а не `bool`. Исходы логина — `enum`, который `match` обязан разобрать целиком. Порядок шагов аутентификации — цепочка типов, где `create_session` физически не вызвать, пока не пройдена проверка. Проверки, что раньше лежали в рантайме россыпью `if`-ов, переехали в систему типов — и компилятор отказывается собирать программу, которая их нарушает.

Обмен честный: типов становится больше, зато ошибки переезжают со стадии «упало в проде» на «не скомпилировалось». Там, где ошибка стоит денег или безопасности, он выгодный — собственно, ровно с этого и начинал Jane Street. А чувство меры — где это оправдано, а где overkill — приходит с практикой.

В Части 2 («Контракты») тот же фундамент уходит в трейты, ассоциированные типы и const generics — то, чем задают _интерфейсы_ для чужого кода. Typestate там становится частью контракта трейта: «реализация обязана провести объект через эти состояния».